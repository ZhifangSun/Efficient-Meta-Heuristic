...
class GeneticScheduler(Scheduler):
    ...
    def schedule(self, app,targetms):
        ...
        while True:
            ...
            if population[0].makespan<targetms or k>=100:
                break
    ...
    # def init_population(self, app, pop_size):   #多条工艺线路
    ...
    #                     or_chromosome.append(random.randint(temp[a]*l,(temp[a]+1)*l-1))
    #                     temp.remove(temp[a])
    #                 chromosome[j].lis.append(or_chromosome)
    ...
    # def init_population(self, app, pop_size):   #使用完全随机数编码
    #     # l = len(ComputingSystem.processors)
    #     chromosomes = []
    #     tasks = app.tasks
    #     OR=list(app.OR.items())
    ...
    #             for k in OR[j][chromosome[j].id]:
    #                 # a=random.randint(0,len(temp)-1)
    #                 chromosome[j].lis.append(random.randint(0,300))
    #                 # temp.remove(temp[a])
    #         for j in range(len(tasks)-app.ORnum):
    #             # print(len(temp))
    #             # a=random.randint(0,len(temp)-1)
    #             # print(a)
    #             # print(len(temp))
    #             chromosome.append(random.randint(0,300))
    #             # temp.remove(temp[a])
    #
    #         chromosomes.append(chromosome)
    #         # if i==0:
        ...
        while len(chromosomes) > 0:
            ...
            for j in range(0, len(chromosome)):  # 遍历排序
                ...
                for p1 in list(computation_time_matrix[task.id-1].keys()):  # 遍历处理器集
                    ...
                    earliest_start_time_of_this_processor = SchedulerUtils.IPPS_calculate_earliest_start_time(task, p, chromosome_dex)  # 当前遍历处理器上最早可用启动时间
    ...
    def get_index(self, chromosome, chromosome_dex, candidate_tasks, size, ctm):#gene个体的基因，scale=1/size
        ...
        for i in range(size):
            # print(i)
            # print(chromosome_dex.index(candidate_tasks[i].id))
            # print("!!")
            # print(len(chromosome))
            # print(len(chromosome_dex))
    ...
    def crossover(self, app, population):
        ...
        for i in range(0, len(population)-1, 2):
            ...
            if ORnum>0:
                crossover_point1 = random.randint(1, ORnum)
                for j in range(crossover_point1):
                    prev_chromosome[j], next_chromosome[j] = next_chromosome[j], prev_chromosome[j]
            crossover_point2=random.randint(ORnum+1,len(population[i].chromosome))
            for j in range(crossover_point2,len(population[i].chromosome)):
                prev_chromosome[j], next_chromosome[j] = next_chromosome[j], prev_chromosome[j]
    ...
    def mutate(self, app, population):
        ...
        for p in population:
            ...
            while (pos2>=ORnum and pos1>=ORnum and pos1==pos2) or (pos2<=ORnum and pos1<=ORnum and pos1==pos2 and len(p[pos1].lis)==1):   #两条工艺线路
                pos2 = random.randint(0, len(p) - 1)
            # print(ORnum)
    ...
    #             if pos1<ORnum:
    #                 sign1=1
    #                 pos1_1=random.randint(0, len(p[pos1].lis)-1)
    #
    #             if sign1==0:
    #                 p[pos1] = random.randint(0,300)
    #             else:
    #                 p[pos1].lis[pos1_1] = random.randint(0,300)
    #         else:
    #             OR = list(app.OR.items())
    #             pos1 = random.randint(0, ORnum-1)
    #             if p[pos1].id==0:
    #                 p[pos1].id=1
    #                 p[pos1].lis=[]
    #                 for k in OR[pos1][1]:
    #                     p[pos1].lis.append(random.randint(0,300))
    #             else:
    #                 p[pos1].id = 0
    #                 p[pos1].lis = []
    #                 for k in OR[pos1][0]:
