...
class tabuIPPSScheduler(Scheduler):
    ...
    def schedule(self, app,targetms):
        ...
        while True:
            ...
            neighborhood = self.find_neighborhood(app,[population[0].chromosome])#前后交叉
            # mutation_chromosomes = self.mutate(app,crossover_chromosomes)#个体变异
            # population = population[:len(population)//2]

            for i in neighborhood:
                if i[2][0].makespan<gBest:
                    gBest=i[2][0].makespan
                    population = i[2]
                    if [i[0],i[1]] not in tabu_list:
                        tabu_list.append([i[0],i[1]])
                        break
            if len(tabu_list)>10:
                tabu_list.pop(0)
    ...
    def find_neighborhood(self, app, population):
        # print(population)
        ORnum = len(app.OR)
        # OR = app.OR  #多工艺线路
        ...
        for i in range(5):
            ...
            neighborhood.append([a,p[a].id])
            neighborhood.append(flag)
            neighborhood.append(population)
            population=[population[0].chromosome]

            neighborhood_list.append(neighborhood)

        # print(neighborhood_list)
        neighborhood_list.sort(key=lambda seq: seq[2][0].makespan)
