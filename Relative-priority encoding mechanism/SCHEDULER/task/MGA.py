...
class MGAScheduler(Scheduler):
    ...
    def schedule(self, app,targetms, targetet, begin_time):
        ...
        while True:
            ...
            crossover_chromosomes = self.crossover(app,half_population)#前后交叉
            ...
            if population[0].makespan<targetms or k>=100 or run_time>=targetet:
                break
    ...
    def select(self, population):
        ...
        for i in range(0, len(population) - 1, 2):
            ...
            while a==b:
                b = random.randint(0, len(population) - 1)
            if population[a].makespan>=population[b].makespan:
                if Pr<=0.8:
                    half_population.append(population[a])
                else:
                    half_population.append(population[b])
            else:
                if Pr<=0.8:
                    half_population.append(population[b])
                else:
    ...
    def crossover(self, app, population):
        ...
        for i in range(0, len(population)-1, 2):
            ...
            ma_chromosome1 = []
            ma_chromosome2 = []
            or_chromosome1.extend(population[i].chromosome[:ORnum])
            or_chromosome2.extend(population[i+1].chromosome[:ORnum])
            prev_chromosome.extend(population[i].chromosome[ORnum:ORnum+len(app.tasks)])
            next_chromosome.extend(population[i+1].chromosome[ORnum:ORnum+len(app.tasks)])
            ma_chromosome1.extend(population[i].chromosome[ORnum+len(app.tasks):])
            ma_chromosome2.extend(population[i+1].chromosome[ORnum+len(app.tasks):])
    ...
    def mutate(self, app, population):
        ...
        for i in range(0, len(population)):
            ...
            op_chromosome = []
            ma_chromosome = []
            or_chromosome.extend(population[i][:ORnum])
            op_chromosome.extend(population[i][ORnum:ORnum + len(app.tasks)])
            ma_chromosome.extend(population[i][ORnum + len(app.tasks):])
            """OR变异"""
            a=random.randint(0,len(or_chromosome)-1)
            if or_chromosome[a]==1:
                or_chromosome[a]=2
            else:
                or_chromosome[a]=1
            ...
            """操作变异"""
            while True:
                pos = random.randint(0, len(op_chromosome) - 1)
                taskid = op_chromosome[pos]
                # print(taskid)
                FirstSUC = app.tasknum
                flag = True
                if taskid.successors:
                    for i in range(pos + 1, app.tasknum):
                        for j in taskid.successors:
                            if j == op_chromosome[i]:
                                FirstSUC = i
                                flag = False
                                break
                        if not flag:
                ...
                k = random.randint(pos + 1, FirstSUC-1)
                taskk = op_chromosome[k]
                fll = True
                ...
                if fll:
                    ...
                    continue
            op_chromosome[pos], op_chromosome[k] = op_chromosome[k], op_chromosome[pos]

            """机器变异"""
            a = random.randint(0, len(ma_chromosome) - 1)
            rand_ma = random.randint(0, len(computation_time_matrix[a]) - 1)
            ma_chromosome[a]=processors[list(computation_time_matrix[a].keys())[rand_ma] - 1]

            merge_chromosome = []
            merge_chromosome += or_chromosome
            merge_chromosome += op_chromosome
            merge_chromosome += ma_chromosome

            newpopulation.append(merge_chromosome)
