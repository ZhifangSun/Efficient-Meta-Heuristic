...
class ApplicationService(object):
    ...
    def init_application(Scheduler,app, task_number, computation_time_matrix, computation_cost_matrix, communication_time_matrix):
        ...
        if Scheduler.scheduler_name == 'HGA' or Scheduler.scheduler_name == 'NGA':
        ...
        # for ii in range(len(app.tasks)):
        #     print(app.tasks[ii].rank_up_value,end=" ")
        ...
        if Scheduler.scheduler_name=='LWSGA' or Scheduler.scheduler_name=='TMGA':
    ...
    def __tag_entry_and_exit_task(app):
        for task in app.tasks:  # 遍历任务集
            if not task.predecessors:  # 如果任务的直接前驱任务集为空, 则
                ...
                app.entry_task.append(task)  # 同时将任务添加至应用的入口任务集
            if not task.successors:  # 如果任务的直接后继任务集为空, 则
                task.is_exit = True  # 置任务为出口任务
                app.exit_task.append(task)  # 同时将任务添加至应用的出口任务集
