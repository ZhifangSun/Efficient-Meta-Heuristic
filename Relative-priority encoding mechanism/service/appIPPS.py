...
class ApplicationService(object):
    ...
    # 初始化应用, 静态方法
    @staticmethod
    def init_application(app, OR_in, not_in_OR, OR, ORnum, task_number, computation_time_matrix, ctm, communication_time_matrix, jo, job_label):
        app.ORnum=ORnum
        app.ctm=ctm
        app.OR=OR
        app.OR_in=OR_in
        app.not_in_OR=not_in_OR
        ApplicationService.__init_task_list(app, task_number, jo, job_label)                       # 初始化任务列表
    ...
    # 初始化任务列表, 静态私有方法
    @staticmethod
    def __init_task_list(app, task_number, jo, job_label):
        ...
        for i in range(task_number):
            ...
            t = Task((i + 1), "%d-%d" % (job_label[job], op))  # 给任务取名 "job-op"
            op+=1
